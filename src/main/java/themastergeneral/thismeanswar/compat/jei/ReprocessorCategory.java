...
public class ReprocessorCategory implements IRecipeCategory<ReprocessorRecipe> {
	...
	public ReprocessorCategory(@Nonnull final IGuiHelper guiHelper) {
		background = guiHelper.createDrawable(new ResourceLocation("jei", "textures/jei/gui/gui_vanilla.png"), 0, 114, 82, 54);
	    icon = guiHelper.createDrawableItemStack(new ItemStack(TMWBlocks.tip_recycler));
	  }
	@Override
	public RecipeType<ReprocessorRecipe> getRecipeType() {
		return new RecipeType<>(new ResourceLocation(TMWMain.MODID, "reprocessor"), ReprocessorRecipe.class);
	}

	@Override
	public Component getTitle() {
		return ModUtils.displayTranslation("thismeanswar.jei.reprocessor");
	}
