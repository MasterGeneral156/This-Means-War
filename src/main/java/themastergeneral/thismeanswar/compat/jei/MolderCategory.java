...
public class MolderCategory implements IRecipeCategory<MolderRecipe> {

	@Nonnull private final IDrawable background;
	@Nonnull private final IDrawable icon;
	
	public MolderCategory(@Nonnull final IGuiHelper guiHelper) {
		background = guiHelper.createDrawable(new ResourceLocation("jei", "textures/jei/gui/gui_vanilla.png"), 0, 114, 82, 54);
	    icon = guiHelper.createDrawableItemStack(new ItemStack(TMWBlocks.tip_molder));
	  }
	@Override
	public RecipeType<MolderRecipe> getRecipeType() {
		return new RecipeType<>(new ResourceLocation(TMWMain.MODID, "molder"), MolderRecipe.class);
	}

	@Override
	public Component getTitle() {
		return ModUtils.displayTranslation("thismeanswar.jei.molder");
	}
	...
	@Override
	public void setRecipe(IRecipeLayoutBuilder builder, MolderRecipe recipe, IFocusGroup focuses) {
